...
function deserializeTx(hexStr) {
  ...
  if (version === -5) {
    ...
    var endEpochCumScTxCommTreeRoot = buf.slice(offset, offset + endEpochCumScTxCommTreeRootLen).toString('hex'); //come back to this one
  ...
  if (version === -5) {
    ...
    for (var _i4 = 0; _i4 < voutLen; _i4++) {
      ...
      var _scriptLen2 = varuint.decode(buf, offset);

      offset += varuint.decode.bytes;

      var _script2 = buf.slice(offset, offset + _scriptLen2);

      offset += _scriptLen2;
      txObj.outs.push({
        satoshis: _satoshis,
        script: _script2.toString('hex'),
