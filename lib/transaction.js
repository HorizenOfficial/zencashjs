...
function multiSignTx(_txObj, i, privKeys, redeemScript) {
  ...
  var signatures = privKeys.map(function (x) {
    ...
    var sigSize = zbufferutils.getPushDataLength(sig);
    return sigSize + sig;
  }).join('');

  txObj.ins[i].script = zopcodes.OP_0 + signatures + zopcodes.OP_PUSHDATA1 + zbufferutils.getPushDataLength(redeemScript) + redeemScript;
