...
function signTx(_txObj, i, privKey) {
  ...
  txObj.ins[i].script = zbufferutils.getStringBufferLength(scriptSig) + scriptSig + zbufferutils.getStringBufferLength(pubKey) + pubKey;
 ...
 * return {String} signed transaction
 */
function multiSignTx(_txObj, i, privKeys, redeemScript) {
  var hashcode = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : zconstants.SIGHASH_ALL;
  ...
  var signatures = privKeys.map(function (x) {
    // sig is the signature
    // 'push 71 bytes' is the size of the signature
    var sig = getScriptSignature(x, signingTx, hashcode);
    var sigSize = zbufferutils.getStringBufferLength(sig);
    return sigSize + sig;
  }).join('');

  txObj.ins[i].script = zopcodes.OP_0 + signatures + zopcodes.OP_PUSHDATA1 + zbufferutils.getStringBufferLength(redeemScript) + redeemScript;
