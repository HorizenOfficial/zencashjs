...
function applyMultiSignatures(_txObj, i, signatures, redeemScript) {
  ...
  var allSigs = signatures.slice();
  // extract applied signatures
  if (txObj.ins[i].script.length > 0) {
    var prevSigs = [];
    var script = txObj.ins[i].script;
    var removedScriptSig = script.replace(redeemScript, '');
    var removedTrails = removedScriptSig.slice(2, -4);
    var sigs = removedTrails;

    while (sigs.length > 0) {
      var sigLen = parseInt(sigs.slice(0, 2), 16).toString(10);
      var sig = Buffer.from(sigs.slice(2), 'hex').slice(0, sigLen).toString('hex');
      prevSigs.push(sig);
      sigs = sigs.slice(2 + sig.length);
    }
    var newSigs = signatures.filter(function (sig) {
      return prevSigs.indexOf(sig) < 0;
    });
    allSigs = newSigs.concat(prevSigs);
  }

  // Fix signature order
  var unmatched = JSON.parse(JSON.stringify(allSigs));
  var orderedSigs = [];
  pubKeys.map(function (pubKey) {
    var keyPair = secp256k1.keyFromPublic(pubKey, 'hex');
    var msg = txToMsg(txObj, i, redeemScript, hashcode);
    var match = void 0;
    unmatched.some(function (sig, i) {
      if (!sig) return false;
      var ver = keyPair.verify(msg, Buffer.from(sig, 'hex').slice(0, -1));
  ...
  var redeemScriptLenght = Buffer.from(redeemScript, 'hex').length;
  var pushDataBuffer = Buffer.alloc(pushdata.encodingLength(redeemScriptLenght));
  pushdata.encode(pushDataBuffer, redeemScriptLenght, 0);

  // http://www.soroushjp.com/2014/12/20/bitcoin-multisig-the-hard-way-understanding-raw-multisignature-bitcoin-transactions/
  txObj.ins[i].script = zopcodes.OP_0 + orderedSigs.map(function (x) {
    return zbufferutils.getPushDataLength(x) + x;
  }).join('') + pushDataBuffer.toString('hex') + redeemScript;
