...
function mkPubkeyHashReplayScript(address) {
  ...
  // Need to reverse it
  var blockHash = '00000004bbe8504b7a8e7c6e23ea6fa57878ce946f9819752405d964175c4276';
  ...
  return opcodes.OP_DUP + opcodes.OP_HASH160 + getStringBufferLength(subAddrHex) + subAddrHex + opcodes.OP_EQUALVERIFY + opcodes.OP_CHECKSIG + blockHashLength + blockHashHex + blockHeightLength + blockHeightHex + opcodes.OP_CHECKBLOCKATHEIGHT;
}
 ...
 */
function signTx(txObj, i, privKey, hashcode) {
  ...
  const signingTxWithHashcode = Buffer.concat([Buffer.from(signingTxHex, 'hex'), _buf16]);
