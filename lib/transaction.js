...
function serializeTx(txObj) {
  ...
  if (txObj.version == -4) {
    ...
    for (var i = 0; i < txObj.vsc_ccout.length; i++) {
      var sc_c = txObj.vsc_ccout[i];
      var _b = Buffer.alloc(12);
      ...
      if (sc_c.vFieldElementCertificateFieldConfig) {
        for (var _i2 = 0; _i2 < sc_c.vFieldElementCertificateFieldConfig.length; _i2++) {
          serializedTx += sc_c.vFieldElementCertificateFieldConfig[_i2].toString(hexRadix);
        }
      }
      ...
      if (sc_c.vBitVectorCertificateFieldConfig) {
        var bitVectorIdx = 0;
        var maxCompressedSizeIdx = 1;
        for (var _i3 = 0; _i3 < sc_c.vBitVectorCertificateFieldConfig.length; _i3++) {
          var _b2 = Buffer.alloc(8);
          _b2.writeUInt32LE(sc_c.vBitVectorCertificateFieldConfig[_i3][bitVectorIdx], 0);
          _b2.writeUInt32LE(sc_c.vBitVectorCertificateFieldConfig[_i3][maxCompressedSizeIdx], 4);
          serializedTx += _b2.toString('hex');
        }
      }
    ...
    for (var i = 0; i < txObj.vft_ccout.length; i++) {
      ...
      serializedTx += _buf32.toString('hex');
      var pubKeyHash = (0, _address.addrToPubKeyHash)(txObj.vft_ccout[i].mcReturnAddress);
 ...
 */
function createRawTx(history, recipients, blockHeight, blockHash, vft) {
  var txObj = {
    locktime: 0,
    version: 1,
    ins: [],
...
function applyMultiSignatures(_txObj, i, signatures, redeemScript) {
  ...
  var txObj = JSON.parse(JSON.stringify(_txObj));
  var pubkeys = [];
  for (var pos = 2; pos < redeemScript.length - 2 * 2;)
  // skipping OP_M, OP_N and OP_CHECKMULTISIG code
  {
    var pubkeyLen = parseInt(redeemScript.slice(pos, pos + 2), 16);
    pos += 2;
    if (pubkeyLen == 0) {
      return "ERROR";
    }
    var pubkey = redeemScript.slice(pos, pos + pubkeyLen * 2);
    pos += pubkeyLen * 2;
    pubkeys.push(pubkey);
  }
  ...
  var signingTxHex = serializeTx(signingTx);
  var signingTxWithHashcode = signingTxHex + _buf16.toString('hex');
  var msg = zcrypto.sha256x2(Buffer.from(signingTxWithHashcode, 'hex')); // Sha256 it twice, according to spec

  // find the association between pubkeys and signatures
  var sortedSignatures = [];
  var pubkeysUncompressed = pubkeys.map(function (x) {
    return secp256k1_.publicKeyConvert(Buffer.from(x, 'hex'), false);
  });
  var _loop = function _loop(pkuId) {
    verifiedSignatureId = signatures.findIndex(function (signature) {
      return secp256k1.verify(msg, signature.slice(0, signature.length - 2), pubkeysUncompressed[pkuId]) === true;
  ...
  for (var pkuId = 0; pkuId < pubkeysUncompressed.length; pkuId++) {
    var verifiedSignatureId;
    _loop(pkuId);
  }
  ...
  txObj.ins[i].script = zopcodes.OP_0 + sortedSignatures.map(function (x) {
