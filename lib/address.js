...
function pubKeyToAddr(pubKey) {
  var envPubKeyHash = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : zconfig.mainnet.pubKeyHash;
  var hash160 = zcrypto.hash160(Buffer.from(pubKey, 'hex'));
  return bs58check.encode(Buffer.from(envPubKeyHash + hash160, 'hex')).toString('hex');
}
/*
...
function pubKeyHashToAddr(pubKeyHash) {
  var envPubKeyHash = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : zconfig.mainnet.pubKeyHash;
  return bs58check.encode(Buffer.from(envPubKeyHash + pubKeyHash, 'hex')).toString('hex');
}
/*
...
function addrToPubKeyHash(addr) {
