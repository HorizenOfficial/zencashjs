...
function deserializeScOutputs(buf, offset) {
  ...
  var constant = '';
  var wCeasedVk = '';
  var provingSystem = "CoboundaryMarlin";
  var numSco = varuint.decode(buf, offset);
  offset += varuint.decode.bytes;

  for (var i = 0; i < numSco; i++) {
    var version = buf.readInt8(offset + 3);
    ...
    offset += 1;
    var certProvingSystem = provingSystem;
    var cswProvingSystem = provingSystem;
    outputs.push({
      n: i,
      version: version,
      ...
      constant: constant,
      cswProvingSystem: cswProvingSystem,
      wCertVk: wCertVk,
      wCeasedVk: wCeasedVk,
