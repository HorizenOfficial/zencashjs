 ...
 */
function mkPubkeyHashScript(address: string){
    var addrHex = bs58check.decode(address).toString('hex')
    var subAddrHex = addrHex.substring(2, addrHex.length) // Cut out the '0'
 ...
 */
function createRawTranscation (history: {txid: string, vout: number, value: number, address: string}[], outs: {satoshis: number, address: string}[]) {
