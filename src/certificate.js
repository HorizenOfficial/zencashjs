...
function deserializeCertFields(buf: Buffer, offset: number) {
  ...
  const endEpochCumScTxCommTreeRoot = buf
    .slice(offset, offset + endEpochCumScTxCommTreeRootLen)
    .toString("hex");
  ...
  for (let i = 0; i < vFieldElementCertificateFieldLen; i++) {
    ...
    vFieldElementCertificateField.push(
      buf.slice(offset, offset + fieldLen).toString("hex")
    );
    offset += fieldLen;
  }
  ...
  for (let i = 0; i < vBitVectorCertificateFieldLen; i++) {
    ...
    vBitVectorCertificateField.push(
      buf.slice(offset, offset + fieldLen).toString("hex")
    );
    offset += fieldLen;
  }
...
function getTotalAmount(outputs): number {
  let totalAmount = 0;
