...
describe("scripts/check-lib-sync.sh", () => {
  ...
  const addTemporaryChange = (dir) =>
    execSync(`touch ${dir}/tmp.js && git add --intent-to-add ${dir}/tmp.js`);
  const revertTemporaryChange = (dir) =>
    execSync(`git reset ${dir}/tmp.js && rm ${dir}/tmp.js`);
